...
def recuperer_liste_actions() -> list:
    ...
    """
    with open(os.path.join('OC_P7', 'AlgoInvest_actions.csv'), 'r') as data:
...
def brute_force(actions_as_dict: list):
    ...
    for i in range(2, len(actions_as_dict) + 1):
        ...
        for panier in paniers:
            ...
            # memory conscious vs sum([action["cout"] for action in panier]) time conscious
            if cout_panier < 500:
...
if __name__ == "__main__":
    ...
    print("- Temps d'exécution pour la fonction de Force Brute:  ", end='')
